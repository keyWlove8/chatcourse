<template>
  <div v-if="show" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
    <div class="bg-white dark:bg-gray-800 rounded-2xl p-8 w-full max-w-md animate-scale-in">
      <!-- 角色头像区域 -->
        ...
        <div class="relative inline-block">
          ...
          <div class="w-32 h-32 mx-auto mb-4 relative">
            <img
              ...
              :src="convertImageUrl(characterAvatar)"
              :alt="characterName"
      ...
      <div class="space-y-4">
        ...
        <div class="text-center">
          <div v-if="isCallActive && isRecording" class="flex items-center justify-center gap-2 text-red-500">
            <div class="w-3 h-3 bg-red-500 rounded-full animate-pulse"></div>
            <span class="text-sm font-medium">正在录音，请说话...</span>
          </div>
          <div v-else-if="isCallActive && isProcessing" class="flex items-center justify-center gap-2 text-blue-500">
            <i class="fa fa-spinner fa-spin"></i>
            <span class="text-sm font-medium">正在处理您的语音...</span>
          </div>
          <div v-else-if="isCallActive && isPlaying" class="flex items-center justify-center gap-2 text-blue-500">
            <i class="fa fa-volume-up"></i>
            <span class="text-sm font-medium">正在播放AI回复...</span>
          </div>
          <div v-else-if="isCallActive && isWaitingForResponse" class="flex items-center justify-center gap-2 text-yellow-500">
            <i class="fa fa-clock"></i>
            <span class="text-sm font-medium">AI正在回复，请稍等...</span>
          </div>
          <div v-else-if="isCallActive && isListening" class="flex items-center justify-center gap-2 text-green-500">
            <div class="w-3 h-3 bg-green-500 rounded-full animate-pulse"></div>
            <span class="text-sm font-medium">正在监听，请说话...</span>
          </div>
          <div v-else class="flex items-center justify-center gap-2 text-gray-500">
            <i class="fa fa-phone"></i>
            <span class="text-sm font-medium">通话已结束</span>
          </div>
        </div>

        <!-- 录音波形动画 -->
        <div v-if="isRecording" class="flex justify-center items-center gap-1 h-8">
          <div v-for="i in 5" :key="i" 
               class="w-1 bg-green-500 rounded-full animate-pulse"
               :style="{ 
        ...
        <div v-if="showWaitingMessage" class="flex justify-center items-center gap-2 text-amber-500 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-lg p-3 mx-4">
          ...
          <span class="text-sm font-medium">请稍等，AI正在回复中...</span>
        </div>
...
const playAiResponse = (audioUrl) => {
  try {
    // 转换音频URL，确保通过代理访问
    const convertedUrl = convertAudioUrl(audioUrl)
